...
- (void)creatDataBase{
    ...
    NSString *creatSQL = [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@ (rowid INTEGER PRIMARY KEY AUTOINCREMENT, keyWord text,subTitle text,currentTime text)",TABLE_NAME];
    char *errorMsg;
...
- (void)insterSearchModel:(SearchModel *)searchModel{
    ...
    NSString *insterSQL = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ (keyWord,subTitle,currentTime) VALUES (?,?,?)",TABLE_NAME];
    char *errorMsg;
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(dataBase, insterSQL.UTF8String, -1, &stmt, nil) == SQLITE_OK) {
        ...
        sqlite3_bind_text(stmt, 2, searchModel.subTitle.UTF8String, -1, nil);
        sqlite3_bind_text(stmt, 3, searchModel.currentTime.UTF8String, -1, nil);
    }
...
- (NSMutableArray *)selectAllSearchModel{
    ...
    if (sqlite3_prepare_v2(dataBase, selecteSQL.UTF8String, -1, &stmt, nil) == SQLITE_OK) {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            NSMutableString *subTitle=[NSMutableString stringWithCString:(char*)sqlite3_column_text(stmt, 2) encoding:NSUTF8StringEncoding];
            NSMutableString *currentTime=[NSMutableString stringWithCString:(char*)sqlite3_column_text(stmt, 3) encoding:NSUTF8StringEncoding];
            SearchModel *searchModel = [SearchModel creatSearchModel:keyWord subTitle:subTitle currentTime:currentTime];
